...
export const CloneSshUrl = {
	...
	prefixDomain(url: CloneSshUrl, prefix: string): CloneSshUrl {
		const { domain, author, repoName } = CloneSshUrl.parse(url)
		return CloneSshUrl.create(prefix + domain, author, repoName)
	},
	...
	parse(url: CloneSshUrl): ParsedCloneSshUrl {
		...
		const [, domain, author, repoName] = matched
